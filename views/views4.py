...
from adminPanel.models import CustomUser, TradingAccount, ActivityLog, Transaction, DemoAccount, CommissionTransaction
    ...
    def get(self, request):
        try:
            ...
            def safe_get_real_balance():
                try:
                    mt5action = MT5ManagerActions()
                    account_ids = [i.account_id for i in TradingAccount.objects.filter(user=request.user).exclude(account_type="prop")]               
            ...
            def safe_get_mam_investments():
                try:
                    return sum([i.amount for i in Transaction.objects.filter(trading_account__user=request.user, transaction_type = "deposit_trading", trading_account__account_type = "mam_investment").exclude(status='pending')])
                except Exception:
                    return 0.0

            def safe_get_mam_managed_funds():
                try:
                    return sum([i.amount for i in Transaction.objects.filter(trading_account__mam_master_account__user = request.user, transaction_type = "deposit_trading", trading_account__account_type = "mam_investment").exclude(status='pending')])
                except Exception:
                    return 0.0
            ...
            try:
                live_accounts = TradingAccount.objects.filter(user=request.user).count()
            except Exception:
                live_accounts = 0
...
class IBTransactionsView(APIView):
    ...
    def get(self, request):
        try:
            # For now, return commission withdrawal transactions from the Transaction model
            # Filter for commission withdrawal transactions for this user
            transactions = Transaction.objects.filter(
                user=request.user,
                transaction_type="commission_withdrawal"
            ).order_by('-created_at').values(
                'amount', 'status', 'created_at', 'trading_account__account_id'
            )
            ...
            data = [
                {
                    ...
                    'trading_account': t['trading_account__account_id'] or 'N/A',
                    'account_id': t['trading_account__account_id'] or 'N/A',
                }
                for t in transactions
            ]
